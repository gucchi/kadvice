...
extern int sc_check_vm_enough_memory(struct mm_struct * mm,long pages);
extern int sc_check_bprm_set_creds(struct linux_binprm * bprm);
extern int sc_check_bprm_check_security(struct linux_binprm * bprm);
...
extern int sc_check_sb_alloc_security(struct super_block * sb);
extern void sc_check_sb_free_security(struct super_block * sb);
...
extern int sc_check_sb_parse_opts_str(char * options,struct security_mnt_opts * opts);
extern int sc_check_path_unlink(struct path * dir,struct dentry * dentry);
extern int sc_check_path_mkdir(struct path * dir,struct dentry * dentry,int mode);
extern int sc_check_path_rmdir(struct path * dir,struct dentry * dentry);
extern int sc_check_path_mknod(struct path * dir,struct dentry * dentry,int mode,unsigned int dev);
extern int sc_check_path_truncate(struct path * path,loff_t length,unsigned int time_attrs);
extern int sc_check_path_symlink(struct path * dir,struct dentry * dentry,const char * old_name);
...
extern int sc_check_inode_alloc_security(struct inode * inode);
extern void sc_check_inode_free_security(struct inode * inode);
...
extern int sc_check_file_alloc_security(struct file * file);
extern void sc_check_file_free_security(struct file * file);
...
extern void sc_check_cred_commit(struct cred * new,const struct cred * old);
...
extern int sc_check_msg_msg_alloc_security(struct msg_msg * msg);
extern void sc_check_msg_msg_free_security(struct msg_msg * msg);
extern int sc_check_msg_queue_alloc_security(struct msg_queue * msq);
extern void sc_check_msg_queue_free_security(struct msg_queue * msq);
...
extern int sc_check_shm_alloc_security(struct shmid_kernel * shp);
extern void sc_check_shm_free_security(struct shmid_kernel * shp);
...
extern int sc_check_sem_alloc_security(struct sem_array * sma);
extern void sc_check_sem_free_security(struct sem_array * sma);
...
extern int sc_check_socket_sock_rcv_skb(struct sock * sk,struct sk_buff * skb);
...
extern int sc_check_sk_alloc_security(struct sock * sk,int family,gfp_t priority);
extern void sc_check_sk_free_security(struct sock * sk);
extern void sc_check_sk_clone_security(const struct sock * sk,struct sock * newsk);
extern void sc_check_sk_getsecid(struct sock * sk,u32 * secid);
...
extern int sc_check_xfrm_policy_alloc_security(struct xfrm_sec_ctx ** ctxp,struct xfrm_user_sec_ctx * sec_ctx);
extern int sc_check_xfrm_policy_clone_security(struct xfrm_sec_ctx * old_ctx,struct xfrm_sec_ctx ** new_ctx);
extern void sc_check_xfrm_policy_free_security(struct xfrm_sec_ctx * ctx);
extern int sc_check_xfrm_policy_delete_security(struct xfrm_sec_ctx * ctx);
extern int sc_check_xfrm_state_alloc_security(struct xfrm_state * x,struct xfrm_user_sec_ctx * sec_ctx,u32 secid);
extern void sc_check_xfrm_state_free_security(struct xfrm_state * x);
extern int sc_check_xfrm_state_delete_security(struct xfrm_state * x);
...
extern int sc_check_xfrm_decode_session(struct sk_buff * skb,u32 * secid,int ckall);
